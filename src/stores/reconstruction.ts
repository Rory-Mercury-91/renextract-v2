...
export const isReconstructing = derived(reconstructionStore, ($store) => $store.isReconstructing);
export const reconstructionProgress = derived(reconstructionStore, ($store) => $store.reconstructionProgress);
export const lastReconstructionResult = derived(reconstructionStore, ($store) => $store.lastResult);
export const lastReconstructionError = derived(reconstructionStore, ($store) => $store.lastError);
export const lastValidation = derived(reconstructionStore, ($store) => $store.lastValidation);
export const reconstructionStats = derived(reconstructionStore, ($store) => ({
  ...
  successRate: $store.totalReconstructions > 0 ? ($store.successfulReconstructions / $store.totalReconstructions) * 100 : 0
}));
...
export const reconstructionActions = {
  ...
  async fixQuotesInFiles(filepath: string): Promise<number> {
    try {
      ...
      if (totalCorrections > 0) {
        console.log(`✅ ${totalCorrections} correction(s) de guillemets appliquée(s)`);
      }
  ...
  async reconstructFile(
    ...
    try {
      ...
      console.log(`🔧 Mode de sauvegarde: ${actualSaveMode}`);
      ...
      if (!isValid) {
        ...
        const errors = validation?.summary.errors || ['Fichiers de traduction invalides'];
      ...
      const response = await apiService.reconstructFile(fileContent, filepath, actualSaveMode);
      ...
      if (response.success && response.save_path) {
        ...
        console.log('✅ Reconstruction réussie:', response.save_path);
  ...
  async openReconstructedFile(): Promise<void> {
    ...
    if (currentState.lastResult?.save_path) {
      try {
        const response = await apiService.openExtractionFile(currentState.lastResult.save_path);
        if (!response.success) {
