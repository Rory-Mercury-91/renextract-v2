...
class UpdateManager:
    ...
    def _get_asset_name_for_os(self, assets: list) -> str | None:
    ...
    def check_for_updates(self) -> dict:
        ...
        """
        try:
            print(f"DEBUG: Checking for updates from {self.latest_release_url}")
            ...
            return {"success": False, "error": f"Erreur de connexion: {e!s}"}
        except (KeyError, ValueError) as e:
            print(f"DEBUG: Failed to parse update data: {e}")
            return {"success": False, "error": f"Erreur de parsing: {e!s}"}
    ...
    def download_update(
        ...
        try:
            ...
            return {"success": False, "error": f"Erreur de téléchargement: {e!s}"}
    ...
    def install_update(self, executable_path: str) -> dict:
        ...
        try:
            ...
            if not os.path.exists(executable_path):
                return {"success": False, "error": "L'exécutable de mise à jour n'existe pas"}
            ...
            # On crée un script batch qui fait le remplacement après la
            # fermeture
            if self.current_os == "windows":
                ...
                with open(update_script, "w", encoding="utf-8") as f:
                    ...
                    f.write('echo Attente de la fermeture de l\'application... >> "%LOG_FILE%"\n')
                    ...
                    f.write('  echo Processus encore actif, attente... >> "%LOG_FILE%"\n')
                    f.write("  echo Processus encore actif, attente...\n")
                    f.write("  timeout /t 2 /nobreak >nul\n")
                    f.write("  goto WAIT_EXIT\n")
                    f.write(")\n")
                    f.write('echo Processus completement termine! >> "%LOG_FILE%"\n')
                    ...
                    f.write("  echo Fichier encore en cours d'utilisation, nouvelle tentative...\n")
                    f.write("  timeout /t 2 /nobreak >nul\n")
                    ...
                    f.write(f'if exist "{backup_exe}" del "{backup_exe}" >nul 2>&1\n')

                    # Supprimer le fichier temporaire
                    f.write(f'if exist "{new_exe_path}" del "{new_exe_path}" >nul 2>&1\n')

                    f.write("echo.\n")
                    f.write('echo Mise a jour installee avec succes! >> "%LOG_FILE%"\n')
                    ...
                    f.write("echo.\n")
                    f.write("echo Appuyez sur une touche pour fermer cette fenetre...\n")
                    f.write("pause >nul\n")
                ...
                # Pour le debug, on peut voir la fenêtre cmd en commentant
                # CREATE_NO_WINDOW
                subprocess.Popen(
                    ["cmd", "/c", update_script],
            ...
            update_script = os.path.join(tempfile.gettempdir(), "update_app.sh")
            ...
            return {"success": False, "error": f"Erreur d'installation: {e!s}"}
    ...
    def should_check_for_updates(self) -> bool:
        ...
        check_interval = (
            self._update_config.get("check_interval_hours", 24) * 3600
