...
def generate_modern_html_report(
    ...
    project_name, language = _get_project_info(selection_info, target_path)
    ...
    issues_by_type_actual = _group_issues_by_type(issues_by_file)

    # Générer les cartes de statistiques
    stats_cards = _generate_stats_cards(
        total_issues, files_analyzed, issues_by_type_actual)

    # Générer les sections d'erreurs par type
    error_sections = _generate_error_sections(issues_by_type_actual)
...
<style>
  ...
  .issue-header{{display:flex;align-items:center;
...
def _get_project_info(selection_info: dict | None, target_path: str) -> tuple:
...
def _group_issues_by_type(issues_by_file: dict[str, list[dict]]) -> dict[str, list[dict]]:
    ...
    return issues_by_type_actual


def _generate_stats_cards(
    total_issues: int, files_analyzed: int, issues_by_type_actual: dict[str, list[dict]],
) -> str:
    ...
    <div class="card">
        ...
        <div class="stat">
          ...
          <span class="stat-value">{len(issues_by_type_actual)}</span>
        </div>
    </div>
    """
    ...
    for issue_type, type_issues in issues_by_type_actual.items():
        ...
        <div class="card">
            ...
            <p style="font-size: 0.9rem; opacity: 0.8; margin: 8px 0 0 0;">
                {count} occurrence(s) • {percentage:.1f}% du total
            </p>
        </div>
        """
...
def _generate_error_sections(issues_by_type_actual: dict[str, list[dict]]) -> str:
    """Génère les sections d'erreurs par type"""
    error_sections = ""
    for issue_type, type_issues in issues_by_type_actual.items():
        description = _get_issue_description(issue_type)
        badge_class = _get_badge_class(issue_type)
        ...
        issues_by_file_type = _group_issues_by_file(type_issues)
        file_sections = _generate_file_sections(issues_by_file_type)

        file_count_text = f"{len(issues_by_file_type)} \
            fichier{'' if len(issues_by_file_type) == 1 else 's'}"
        error_sections += f"""
...
def _group_issues_by_file(type_issues: list[dict]) -> dict[str, list[dict]]:
    """Groupe les problèmes par fichier pour un type donné"""
    issues_by_file_type = {}
    ...
    return issues_by_file_type


def _generate_file_sections(issues_by_file_type: dict[str, list[dict]]) -> str:
    """Génère les sections de fichiers"""
    file_sections = ""
    for file_path, file_issues in issues_by_file_type.items():
        file_name = os.path.basename(file_path)
        issue_items = _generate_issue_items(file_issues, file_path)
...
def _generate_issue_items(file_issues: list[dict], file_path: str) -> str:
    """Génère les éléments d'issues pour un fichier"""
    issue_items = ""
    for issue in file_issues:
        line_number = issue.get("line_number", "?")
        message = issue.get("message", "Aucune description")
        content_comparison = _generate_content_comparison(issue)

        issue_items += f"""
        <div class="issue-item">
            <div class="issue-header">
                <h4 class="issue-line">Ligne {line_number}</h4>
                <span class="open-in-editor"
                      onclick="openInEditor('{file_path}', {issue.get("line_number", 0)})">
