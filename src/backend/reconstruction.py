...
def fix_translation_errors_in_txt(filepath: str) -> int:
    ...
    try:
            ...
            for line in f:
                ...
                if '[' in fixed_line and ']' in fixed_line:
                    ...
                    for pattern, replacement in deepl_patterns:
                        ...
                        if matches:
                            fixed_line = re.sub(
                                pattern, replacement, fixed_line)
                ...
                if '«' in fixed_line or '»' in fixed_line:
                    ...
                    fixed_line = fixed_line.replace('»', '\\"')
                    corrections += original_line.count('«') + \
                        original_line.count('»')
        ...
        if corrections > 0:
            ...
            logger.info(
                f"✅ {corrections} correction(s) appliquée(s) à %s", os.path.basename(filepath))
...
class FileReconstructor:
    ...
    def load_file_content(self, file_content: List[str], original_path: str):
        ...
        if os.path.exists(placeholders_path):
            logger.info(
                "📂 Chargement fichier avec placeholders: %s", placeholders_path)
    ...
    def _load_data_for_reconstruction(self):
        ...
        try:
            ...
            logger.info("📂 Chargement : %s lignes (dialogue:%s, astérisques:%s, tildes:%s)",
                        total_lines, total_dialogue, total_asterix, total_tilde)
    ...
    def _build_translation_mapping(self):
        ...
        try:
            ...
            if hasattr(self, 'duplicate_originals_ordered') and self.duplicate_originals_ordered:
                logger.info(
                    f"Utilisation de duplicate_originals_ordered sauvegardée")
                ...
                for content in self.all_contents_linear:
                    content_counts[content] = content_counts.get(
                        content, 0) + 1
                unique_originals = [
                    c for c, count in content_counts.items() if count == 1]
            else:
                # Fallback: ancienne méthode (peut être incorrecte si le fichier doublons a été modifié)
                logger.warning(
                    f"duplicate_originals_ordered non trouvée, utilisation de la méthode de calcul (peut être incorrecte)")
                content_counts = OrderedDict()
                for content in self.all_contents_linear:
                    content_counts[content] = content_counts.get(
                        content, 0) + 1
            ...
            logger.info(
                f"Mapping de traduction créé: {len(self.translation_map)} entrées")
    ...
    def reconstruct_file(self, save_mode: str = 'new_file') -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "✅ Reconstruction réussie en %s", self.reconstruction_time)
    ...
    def _replace_dialogue_placeholders(self, content: List[str]) -> List[str]:
        ...
        try:
            ...
            logger.info(
                "✅ %s lignes de dialogue reconstruites", len(self.line_to_content_indices))
            return content
