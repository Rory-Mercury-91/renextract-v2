<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      if (wslResult.success && wslResult.info?.is_wsl) {
...
{#if showInfo && wslInfo}
  <div class="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
    <div class="flex items-start justify-between">
      <div class="flex-1">
        <h3 class="mb-2 flex items-center text-sm font-semibold text-blue-800 dark:text-blue-200">
        ...
        {#if zenityInfo?.available}
          <div class="mb-2 flex items-center text-xs text-green-700 dark:text-green-300">
            <span class="mr-1">✅</span>
            <span>zenity est installé ({zenityInfo.version}) - Dialogues graphiques disponibles</span>
          </div>
        {:else}
          <div class="mb-3">
            <div class="mb-2 flex items-center text-xs text-orange-700 dark:text-orange-300">
              <span class="mr-1">⚠️</span>
              <span>zenity n'est pas installé - Dialogues graphiques non disponibles</span>
            </div>
        ...
        <div class="text-xs text-blue-600 dark:text-blue-400">
          ...
          <ul class="ml-4 list-disc space-y-1">
            <li>Les chemins Windows sont accessibles via /mnt/c/, /mnt/d/, etc.</li>
            <li>Vous pouvez saisir les chemins manuellement si zenity n'est pas installé</li>
            <li>Les dialogues de fichier fonctionneront avec des prompts texte</li>
          </ul>
        </div>
      </div>
